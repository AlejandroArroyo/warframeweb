// Genera SQL para insertar reliquias en PostgreSQL (Neon)
const RELICS = [
...
console.log('-- Warframe Void Relics - Seed script (idempotente)');
console.log('-- Fuente: https://warframe.fandom.com/wiki/Void_Relic');
console.log('-- Pegar esto en el SQL editor de Neon desde Railway Dashboard');
console.log('');
console.log('-- 1. Ver cuántas hay actualmente:');
console.log('SELECT COUNT(*) AS total_actual FROM "Relic";');
console.log('');
console.log('-- 2. Insertar solo las que faltan (idempotente):');
const values = RELICS.map(r => `  ('${r.era}', E'${r.name}')`);
console.log(`INSERT INTO "Relic" (id, era, name)`);
console.log(`SELECT gen_random_uuid()::text, v.era, v.name FROM (VALUES`);
console.log(values.join(',\n'));
console.log(`) AS v(era, name)`);
console.log(`WHERE NOT EXISTS (SELECT 1 FROM "Relic" r WHERE r.era = v.era AND r.name = v.name);`);
console.log('');
console.log('-- 3. Ver total después del seed:');
console.log('SELECT COUNT(*) AS total_final FROM "Relic";');
